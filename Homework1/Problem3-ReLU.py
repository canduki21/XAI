# =========================================================
# MNIST Second-Order Explanations — ReLU ONLY
# =========================================================
...
test_loader = DataLoader(test_dataset, batch_size=1, shuffle=True)  # REQUIRED
...
# -------------------------
# Select first correctly classified test image
# -------------------------
model.eval()
x0, y0 = None, None
...
with torch.no_grad():
    for x, y in test_loader:
        x, y = x.to(device), y.to(device)
        if model(x).argmax(dim=1).item() == y.item():
            x0 = x.clone()
            y0 = y.item()
            break

assert x0 is not None, "No correctly classified image found"
...
plt.title("ReLU Hessian Eigenvalue Distribution")
